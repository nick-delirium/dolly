<!-- dolly plan · created 2026-08-07T20:19:39Z · @rjshrjndrn -->
# Plan — pi hook extension for auto-inject and auto-log
...
ship a pi extension (.ts using ExtensionAPI) that dolly install writes, giving pi the same ambient behavior Claude has: inject dolly context on session start and auto-log a step per finished turn — by shelling the existing `dolly hook session-start` and `dolly hook stop` commands from pi's before_agent_start and turn_end events. Follow-up to 0003.
